...
interface TaskLibraryClientProps {
  initialTasks: Task[];
}

export function TaskLibraryClient({ initialTasks }: TaskLibraryClientProps) {
  ...
  const taskCategories = useMemo(
    () => ["All", ...Array.from(new Set(initialTasks.map((task) => task.category))).sort()],
    [initialTasks]
  );
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-sm text-text-secondary mt-0.5">
            Browse {initialTasks.length} published engineering task
            {initialTasks.length !== 1 ? "s" : ""} across {taskCategories.length - 1} categories.
          </p>
        </div>
