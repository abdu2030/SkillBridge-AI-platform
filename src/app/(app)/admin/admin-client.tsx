...
type DbPublishStatus = "draft" | "published" | "archived";
...
const initialAdminTasks: AdminTask[] = sampleTasks.slice(0, 4).map((task, index) => ({
  ...
  publishStatus: index === 3 ? "draft" : "published",
...
function statusFromDatabase(status: DbPublishStatus): AdminTaskStatus {
  if (status === "published") return "Published";
  if (status === "archived") return "Archived";
  return "Draft";
}

function statusToDatabase(status: AdminTaskStatus): DbPublishStatus {
  if (status === "Published") return "published";
  if (status === "Archived") return "archived";
  return "draft";
}

function difficultyFromDatabase(value: string): TaskDifficulty {
...
interface DbAdminTaskRow {
  ...
  task_files?: { content: string | null; sort_order: number | null }[] | null;
}

function mapDbAdminTask(task: DbAdminTaskRow): AdminTask {
  return {
    id: task.slug,
    databaseId: task.id,
    slug: task.slug,
    title: task.title,
    category: task.category,
    difficulty: difficultyFromDatabase(task.difficulty),
    estimatedMinutes: task.estimated_minutes,
    tags: task.tags ?? [],
    status: statusFromDatabase(task.status),
    publishStatus: task.status,
    summary: task.summary,
    instructions: task.instructions,
    starterCode:
      task.task_files?.slice().sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0))[0]
        ?.content ?? "",
    submissions: 0,
  };
}
    ...
    async function loadTasks() {
      try {
        ...
        const { data, error } = await supabase
          .from("tasks")
          .select(
            "id, slug, title, summary, instructions, category, difficulty, estimated_minutes, tags, status, task_files(content, sort_order)"
          )
          .order("created_at", { ascending: false });

        if (error || cancelled || !data?.length) return;

        setAdminTasks((data as DbAdminTaskRow[]).map(mapDbAdminTask));
      } catch {
        // Keep local sample tasks available when Supabase is not reachable in development.
      }
    }
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const nextTask: AdminTask = {
      ...
      tags: form.tags
        ...
        .filter(Boolean),
      status: form.status,
      publishStatus: statusToDatabase(form.status),
  ...
  function changePublishStatus(task: AdminTask, status: DbPublishStatus) {
    ...
    if (!task.databaseId) {
      setAdminTasks((current) =>
        current.map((item) =>
          item.id === task.id
            ? { ...item, publishStatus: status, status: statusFromDatabase(status) }
            : item
        )
      );
      setTableMessage("Local sample task status updated. Database tasks publish permanently.");
      return;
    }

    startPublishingTransition(async () => {
      const result = await updateTaskPublishStatus(task.databaseId as string, status);
            ...
            content: (
              <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_420px]">
                ...
                <div className="order-2 space-y-4 xl:order-1">
                  ...
                  {tableMessage && <p className="text-xs text-text-secondary">{tableMessage}</p>}
                                ...
                                <div className="flex items-center justify-end gap-1">
                                  <button
                                    ...
                                    onClick={() =>
                                      changePublishStatus(
                                        task,
                                        task.publishStatus === "published" ? "draft" : "published"
                                      )
                                    }
                                    className="rounded-md border border-border px-2 py-1 text-xs font-medium text-text-secondary transition-colors hover:bg-surface-hover hover:text-text disabled:opacity-50"
                                  >
                                    {task.publishStatus === "published" ? "Unpublish" : "Publish"}
                                  </button>
                                  <Link
                                    href={`/tasks/${task.id}`}
                                    className="rounded-md p-1.5 text-text-tertiary transition-colors hover:bg-surface-hover hover:text-text"
